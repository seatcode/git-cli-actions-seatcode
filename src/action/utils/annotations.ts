...
export async function setAnnotations(resultsPath: any) {
  ...
  const assertionResultsByUrl = mapValues(
    groupBy(assertionResults, 'url'),
    assertions => {
  ...
  Object.entries(assertionResultsByUrl).forEach(([url, assertions]) => {
    ...
    const assertionsText = assertions.map(a => {
      ...
      return (
        `${emoji} \`${a.auditId}${
          a.auditProperty ? '.' + a.auditProperty : ''
        }\` ` +
        `${a.level === 'error' ? 'failure' : 'warning'} for \`${
          a.name
        }\` assertion` +
        `${
