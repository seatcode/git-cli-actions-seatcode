...
export function getInput() {
  ...
  const serverBaseUrl =
    core.getInput('serverBaseUrl') || core.getInput('upload.serverBaseUrl')
  const serverToken =
    core.getInput('serverToken') || core.getInput('upload.token')
  ...
  const temporaryPublicStorage =
    core.getInput('temporaryPublicStorage') === 'true' ? true : false
  if (serverBaseUrl && temporaryPublicStorage) {
    core.setFailed(
      `Both LHCI server and Temporary storage are set, choose one upload method.`
    )
    process.exit(1)
  }
  ...
  return {
    ...
    runs: core.getInput('runs')
      ...
      : numberOfRuns || 1, // `runs`, check config, and fallback to 1
    staticDistDir,
    // assert
...
export function hasAssertConfig(configPath: any) {
